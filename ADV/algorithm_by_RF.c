...
float sum_X2 = 83325; // WARNING: you MUST recalculate this sum if you changed either ST or FS above!
...
int32_t BUFFER_SIZE = FS*ST; // Number of smaples in a single batch
int32_t FS60 = 25*60;  // Conversion factor for heart rate from bps to bpm
int32_t LOWEST_PERIOD = (25*60)/180; //FS60/MAX_HR; // Minimal distance between peaks
int32_t HIGHEST_PERIOD = (25*60)/40; //FS60/MIN_HR; // Maximal distance between peaks
float mean_X = (float)(100-1)/2.0; // Mean value of the set of integers from 0 to BUFFER_SIZE-1. For ST=4 and FS=25 it's equal to 49.5.
